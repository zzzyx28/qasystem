...
async def ensure_bootstrap_admin(session: AsyncSession) -> None:
    """若配置了 BOOTSTRAP_ADMIN_*，则保证该账号存在且为系统管理员。"""
    ...
    if user is None:
        ...
        logger.info("已创建引导系统管理员账号: %s", username)
        return
    ...
    if user.role != ROLE_ADMIN:
        ...
        await session.commit()
        logger.info("已将引导账号设为系统管理员并更新密码: %s", username)
        return

    # 已是系统管理员：按需重置密码（便于运维恢复）
    user.hashed_password = bootstrap_hash
    await session.commit()
    logger.info("已更新引导系统管理员密码: %s", username)
